...
public class GenerationTask implements Runnable {
    ...
    public void run() {
        ...
        while (!stopped && chunkIterator.hasNext()) {
            ...
            final CompletableFuture<Void> chunkFuture = isChunkGenerated
            ...
            // Add to pending set BEFORE attaching completion handler to avoid race condition
            pendingFutures.add(chunkFuture);
            // Run callback on dedicated executor to avoid server thread congestion and deadlock
            chunkFuture.whenCompleteAsync((ignored, throwable) -> {
                try {
                    if (throwable != null) {
                        chunky.getServer().getConsole().sendMessage("Chunk generation failed at " + chunk.x() + "," + chunk.z() + ": " + throwable.getMessage());
                    }
                    update(chunk.x(), chunk.z(), throwable == null);
                } finally {
                    working.release();
                    pendingFutures.remove(chunkFuture);
                }
            }, callbackExecutor);
        }
        // Wait for all pending chunk operations to complete before exiting
        if (!pendingFutures.isEmpty()) {
            chunky.getServer().getConsole().sendMessage("Waiting for " + pendingFutures.size() + " pending chunks to complete...");
            try {
                final CompletableFuture<Void> allFutures = CompletableFuture.allOf(pendingFutures.toArray(new CompletableFuture[0]));
                allFutures.get(60, TimeUnit.SECONDS);
            } catch (TimeoutException e) {
                // Don't cancel - let chunks complete in background
                final long stillPending = pendingFutures.stream().filter(f -> !f.isDone()).count();
                chunky.getServer().getConsole().sendMessage("Warning: " + stillPending + " chunks still loading after 60s timeout");
                chunky.getServer().getConsole().sendMessage("Task will exit, chunks will continue loading in background");
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                chunky.getServer().getConsole().sendMessage("Interrupted while waiting for pending chunks");
            } catch (ExecutionException e) {
                chunky.getServer().getConsole().sendMessage("Error waiting for pending chunks: " + e.getMessage());
            }
        }
