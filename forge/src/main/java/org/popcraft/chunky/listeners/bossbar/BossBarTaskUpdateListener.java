...
public class BossBarTaskUpdateListener implements Consumer<GenerationTaskUpdateEvent> {
    ...
    public void accept(final GenerationTaskUpdateEvent event) {
        ...
        // Boss bar operations must run on server thread for thread safety
        server.execute(() -> updateBossBar(server, barIdentifier, worldIdentifier, chunky, task));
    }

    private void updateBossBar(final MinecraftServer server, final ResourceLocation barIdentifier,
                               final ResourceLocation worldIdentifier, final Chunky chunky, final GenerationTask task) {
